...
//
// ПАТЧИ ДЛЯ MARZBAN API:
// - decodeMarzbanDateTime: исправляет парсинг времени без timezone
// - OptNextPlanModel.Decode: исправляет обработку null значений
//
// Генерация отключена в gen.go чтобы сохранить исправления!
...
// decodeMarzbanDateTime decodes datetime from Marzban API which may not have timezone info
func decodeMarzbanDateTime(d *jx.Decoder) (time.Time, error) {
	str, err := d.Str()
	if err != nil {
	...
	// Try without timezone (Marzban format)
	if t, err := time.Parse("2006-01-02T15:04:05.999999", str); err == nil {
		return t.UTC(), nil // Assume UTC if no timezone
	}
	...
	return time.Time{}, errors.Errorf("unable to parse datetime: %s", str)
}
